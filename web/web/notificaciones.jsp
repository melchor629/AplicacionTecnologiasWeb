...
<body>
    ...
    <div class="container">
        ...
        <div class="col-sm-6">
            ...
            <div class="peticion-amigo row">
                ...
                <div class="col-xs-9 col-sm-10 col-lg-10">
                    <h3>@<%= p.getUsuario().getNombreUsuario()%>
                    </h3>
                </div>

                <div class="col-xs-12 forms">
                    <div class="clearfix">
                        <form id="rechazar-<%=p.getUsuario().getId()%>" class="pull-right form-horizontal" role="form"
                              action="<%= cpath%>/AmistadHandlerServlet" method="POST">
                            <input type="hidden" name="tipo" value="rechazar">
                            <input type="hidden" name="iduser" value="<%=p.getUsuario().getId()%>">
                            <button type="submit" form="rechazar-<%=p.getUsuario().getId()%>"
                                    class="btn btn-default btn-raised">
                                <i class="fa fa-user-times"></i> Rechazar
                            </button>
                        </form>

                        <form id="aceptar-<%=p.getUsuario().getId()%>" class="pull-right form-horizontal" role="form"
                              action="<%= cpath%>/AmistadHandlerServlet" method="POST">
                            <input type="hidden" name="tipo" value="aceptar">
                            <input type="hidden" name="iduser" value="<%=p.getUsuario().getId()%>">
                            <button type="submit" form="aceptar-<%=p.getUsuario().getId()%>"
        ...
        <div class="col-sm-6">
            ...
            <% for (Mensaje mensaje : mensajesNo) { %>
                ...
                <div class="col-xs-12">
                    ...
                    <div class="text-right">
                        <a class="btn btn-primary btn-raised"
                           href="<%=cpath%>/MensajeLeidoServlet?id=<%= mensaje.getId()%>&emisor=<%=mensaje.getIdEmisor().getId()%>">RESPONDER</a>
                        <a class="btn btn-default btn-raised"
                           href="<%=cpath%>/MensajeLeidoServlet?id=<%=mensaje.getId()%>&emisor=<%=mensaje.getIdEmisor().getId()%>&descartar=true">DESCARTAR</a>
                    </div>
                </div>
            </div>
        <% } %>
